...
Antes de montar os modelos, realizamos a preparação dos dados. Todas essas etapas foram realizadas no arquivo `preparacao_dados.ipynb` que se encontra na pasta `notebooks` do repositório e documentado em `docs/docs/Sprints/Sprint 2/Preparação de Dados/preparacao_dados.md`. Como resutado final, temos um dataset pronto para ser utilizado nos modelos. Além disso, o notebook com a criação do modelo se encontra na pasta `notebooks` no arquivo `ModelosRNN.ipynb`.
